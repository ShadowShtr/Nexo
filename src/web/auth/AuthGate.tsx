...
export function AuthGate({area,demo,children}:{area:Area;demo:boolean;children:ReactNode}) {
  ...
  if (state === 'denied') return shell(<><LockKeyhole size={28}/><h1>{t('authDenied')}</h1><p>{t('authDeniedBody')}</p><Button onClick={()=>void client?.auth.signOut({scope:'local'})}>{t('signOut')}</Button></>);
