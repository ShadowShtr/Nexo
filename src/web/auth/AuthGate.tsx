...
export function AuthGate({area,demo,children}:{area:Area;demo:boolean;children:ReactNode}) {
  ...
  const language = <label className="pm-language"><img className="pm-language-flag" src={i18n.language === 'en' ? '/flag-usa.webp' : '/flag-portugal.webp'} alt="" aria-hidden="true"/><span className="pm-sr-only">{t('language')}</span><select aria-label={t('language')} value={i18n.language} onChange={event=>void i18n.changeLanguage(event.target.value)}><option value="pt-PT">Português</option><option value="en">English</option></select></label>;
