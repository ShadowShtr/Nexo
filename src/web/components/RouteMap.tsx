...
export function RouteMap({ route, language, mode = 'full', previewMessage }: RouteMapProps) {
  ...
  return <section className="pm-route-preview" aria-label={language === 'en' ? 'Route preview' : 'Pré-visualização do percurso'}>
    ...
    {mode === 'preview' ? <div className="pm-route-preview-message"><strong>{language === 'en' ? 'Your pickup point' : 'O seu ponto de partida'}</strong><span>{previewMessage ?? (language === 'en' ? 'Choose a destination to calculate the route.' : 'Escolha um destino para calcular o percurso.')}</span></div> : <><div className="pm-route-summary"><strong>{km} km</strong><span>{route.minutes} min</span><span>{language === 'en' ? 'Fictional route' : 'Rota fictícia'}</span></div><ol className="pm-route-points">{route.points.map((point,index)=><li key={`${point.label}-${index}`}><span>{index + 1}</span><div><strong>{point.label}</strong><small>{point.kind === 'pickup' ? (language === 'en' ? 'Pickup' : 'Recolha') : point.kind === 'stop' ? (language === 'en' ? 'Stop' : 'Paragem') : (language === 'en' ? 'Destination' : 'Destino')}</small></div></li>)}</ol><p className="pm-secondary pm-small">{language === 'en' ? 'Illustrative geometry and estimates. The production version will use a road-routing provider.' : 'Geometria e estimativas ilustrativas. A versão funcional usará um fornecedor de rotas rodoviárias.'} {' '}<a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">© OpenStreetMap</a>.</p></>}
  </section>;
}
