...
const tourOptions = [
  ...
  { id: 'sintra', icon: '/compass.webp', pt: 'Sintra', en: 'Sintra', detailPt: 'Palácios, serra e mar', detailEn: 'Palaces, hills and sea' },
...
const clientIcons = {
  location: '/location-pin.webp',
  clock: '/clock.webp',
  calendar: '/calendar.webp',
  car: '/vehicle-sedan.webp',
};
...
export default function CustomerDiscoverSandbox() {
  ...
  const localSuggestions: Suggestion[] = localActiveSuggestions.map(place => ({ ...place, title: localUnitSuggestions.length ? place.title : titleWithUnit(place.title, activeUnit) }));
  const activeSuggestions: Suggestion[] = [...localSuggestions, ...remoteActiveSuggestions
  ...
  useEffect(() => {
    ...
    if (!activeSearch || query.length < 3) {
      setRemoteSuggestions([]);
  ...
  }, [activeQuery, activeSearch?.kind, activeSearch?.index, i18n.language]);
  ...
  return <div className="pm-client-home">
    <header className="pm-client-toolbar">
      <span className="pm-client-wordmark"><span className="pm-client-mark">pm.</span></span>
      <label className="pm-client-language"><img className="pm-language-flag" src={i18n.language === 'en' ? '/flag-usa.webp' : '/flag-portugal.webp'} alt="" aria-hidden="true"/><span className="pm-sr-only">{say('Idioma', 'Language')}</span><select aria-label={say('Idioma', 'Language')} value={i18n.language} onChange={event => changeLanguage(event.target.value)}><option value="pt-PT">Português</option><option value="en">English</option></select></label>
    </header>
