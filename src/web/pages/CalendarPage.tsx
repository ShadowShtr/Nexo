...
export default function CalendarPage() {
  ...
  return <><div className="pm-calendar-toolbar"><label className="pm-date-label">{t('date')}<input aria-label={t('date')} type="date" value={date} onChange={e => { setDate(e.target.value); if(e.target.value) calendar.current?.getApi().gotoDate(e.target.value); }}/></label><label className="pm-switch"><input type="checkbox" checked={sample} onChange={e => { setSample(e.target.checked); if(e.target.checked) { setDate(SAMPLE_DATE); calendar.current?.getApi().gotoDate(SAMPLE_DATE); } }}/>{t('sample')}</label></div>
    ...
    <div className="pm-card pm-calendar"><div className="pm-calendar-controls"><div className="pm-actions"><Button variant="ghost" aria-label={t('previous')} onClick={() => calendar.current?.getApi().prev()}><ChevronLeft size={18}/></Button><Button variant="ghost" onClick={() => calendar.current?.getApi().today()}><CalendarDays size={17} aria-hidden="true" />{t('today')}</Button><Button variant="ghost" aria-label={t('next')} onClick={() => calendar.current?.getApi().next()}><ChevronRight size={18}/></Button></div><strong aria-live="polite">{title}</strong><select aria-label={t('calendar')} value={view} onChange={e => setView(e.target.value)}>{[['timeGridDay', 'day'], ['timeGridWeek', 'week'], ['dayGridMonth', 'month'], ['listDay', 'list']].map(([value, key]) => <option key={value} value={value}>{t(key)}</option>)}</select></div>
    <div className="pm-calendar-scroll"><FullCalendar ref={calendar} plugins={[dayGridPlugin, timeGridPlugin, listPlugin, luxonPlugin]} initialView={view} locales={[ptLocale, enLocale]} eventTimeFormat={{ hour: '2-digit', minute: '2-digit', hour12: false }} locale={i18n.language === 'en' ? 'en-gb' : 'pt'} timeZone="Europe/Lisbon" firstDay={1} headerToolbar={false} events={events} eventContent={info => <span className={info.view.type === 'dayGridMonth' ? 'pm-calendar-month-event' : 'pm-calendar-event'} title={info.event.title}>{info.event.display !== 'background' && <span className="pm-calendar-event-time">{info.timeText}</span>}<span className="pm-calendar-event-title">{info.event.title}</span></span>} editable={false} allDaySlot={false} slotDuration="01:00:00" slotMinTime="06:00:00" slotMaxTime="23:00:00" scrollTime="08:00:00" height={window.innerWidth < 768 ? 'auto' : 590} nowIndicator eventInteractive noEventsContent={t('noEvents')} datesSet={info => { setTitle(info.view.title); const local = new Intl.DateTimeFormat('en-CA', {timeZone:'Europe/Lisbon',year:'numeric',month:'2-digit',day:'2-digit'}).format(info.view.calendar.getDate()); setDate(local); }} eventClick={info => { if(info.event.id !== 'buffer') { trigger.current = info.el; setDetail(info.event.id as 'transfer' | 'tourExample'); dialog.current?.showModal(); } }}/></div></div>
