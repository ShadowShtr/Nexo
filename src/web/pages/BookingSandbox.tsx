...
export function BookingSandbox() {
  ...
  const changeDestination = (value: string) => {
    ...
    if (match >= 0) { setRoute(match); setService('transfer'); setOrigin(transferRoutes[match].points[0].label); setStops([]); }
  };
  const useLocation = () => {
