...
import { Bell, BellRing, CalendarDays, CalendarPlus, ChevronRight, ClipboardList, MessageCircle, Settings2, Share2, WalletCards } from 'lucide-react';
...
export function OwnerHomeSandbox() {
  ...
  return <div className="pm-owner-dashboard">
    ...
    <section className="pm-owner-section"><div className="pm-owner-section-heading"><h2>{say('Resumo do dia', 'Today at a glance')}</h2><a href="#/owner/calendar">{say('Abrir agenda', 'Open calendar')}<ChevronRight size={16}/></a></div><div className="pm-owner-metrics"><article><span className="pm-owner-metric-icon pm-owner-metric-pink"><CalendarDays size={20}/></span><div><strong>{scheduled}</strong><span>{say('Marcações', 'Bookings')}</span><small>{say('Hoje', 'Today')}</small></div></article><article><span className="pm-owner-metric-icon pm-owner-metric-green"><WalletCards size={20}/></span><div><strong>{money(revenue, i18n.language)}</strong><span>{say('Faturação prevista', 'Expected revenue')}</span><small>{say('Hoje', 'Today')}</small></div></article><article><span className="pm-owner-metric-icon pm-owner-metric-amber"><BellRing size={20}/></span><div><strong>{attention.length}</strong><span>{say('Lembretes', 'Reminders')}</span><small>{say('A acompanhar', 'To follow up')}</small></div></article><article><span className="pm-owner-metric-icon pm-owner-metric-blue"><ClipboardList size={20}/></span><div><strong>{activeRequests.length}</strong><span>{say('Pedidos pendentes', 'Pending requests')}</span><small>{say('Vindos do cliente', 'From customers')}</small></div></article></div></section>
    <section className="pm-owner-section"><div className="pm-owner-section-heading"><h2>{say('Precisa da sua atenção', 'Needs your attention')}</h2><a href="#/owner/bookings">{say('Ver todos', 'View all')}<ChevronRight size={16}/></a></div><div className="pm-owner-attention-list">{attention.length ? attention.map(item => <a className="pm-owner-attention-row pm-demo-record" href="#/owner/bookings" key={item.id}><span className="pm-owner-row-icon"><Bell size={16}/></span><span className="pm-owner-row-copy"><strong>{item.name}</strong><small>{new Intl.DateTimeFormat(locale, { day: 'numeric', month: 'short' }).format(new Date(item.date))} · {item.route}</small><small>{item.detail}</small></span><span className="pm-owner-row-message" aria-label={say('Abrir pedido', 'Open request')}><MessageCircle size={17}/></span><ChevronRight className="pm-owner-row-chevron" size={18}/></a>) : <p className="pm-owner-empty">{say('Nenhum pedido pendente.', 'No pending requests.')}</p>}</div></section>
    <section className="pm-owner-section"><div className="pm-owner-section-heading"><h2>{say('Atalhos rápidos', 'Quick shortcuts')}</h2></div><div className="pm-owner-shortcuts"><a href="#/owner/bookings"><span><CalendarPlus size={21}/></span><strong>{say('Nova marcação', 'New booking')}</strong><small>{say('Registar um pedido por telefone ou WhatsApp', 'Record a phone or WhatsApp request')}</small><ChevronRight size={17}/></a><a href="#/customer/discover"><span><Share2 size={21}/></span><strong>{say('Link da página', 'Page link')}</strong><small>{say('Abrir a experiência pública do cliente', 'Open the customer experience')}</small><ChevronRight size={17}/></a><a href="#/owner/settings"><span><Settings2 size={21}/></span><strong>{say('Configurar operação', 'Configure operation')}</strong><small>{say('Valores, regras e publicação', 'Prices, rules and publishing')}</small><ChevronRight size={17}/></a></div></section>
  </div>;
}
