...
export default function CalendarSandbox({ driverOnly = false }: { driverOnly?: boolean }) {
  ...
  return <div className="pm-agenda-workspace">
    ...
    <div className="pm-calendar-toolbar pm-agenda-filters">
      ...
      {!driverOnly && <Button onClick={()=>{setDraft({id:`TEST-${crypto.randomUUID().slice(0,8)}`,driverId:filter==='all'?'0':filter,vehicleId:vehicle==='all'?'0':vehicle,status:'confirmed',from:'Lisboa',to:'Cascais',startsAt:toISO(date + 'T18:00'),endsAt:toISO(date + 'T19:00')});setError('');setFeedback('');}}><Plus size={17} aria-hidden="true" />{say('Nova viagem de teste','New test trip')}</Button>}
    </div>
    ...
    {draft && <form ref={formRef} key={draft.id} className="pm-card pm-demo-form" aria-label={say('Editar viagem de teste','Edit test trip')} onSubmit={e=>{
      ...
      {error && <p role="alert">{error}</p>}<div className="pm-actions"><Button type="submit"><Save size={17} aria-hidden="true" />{say('Guardar teste','Save test')}</Button><Button type="button" variant="secondary" onClick={()=>setDraft(null)}><X size={17} aria-hidden="true" />{say('Fechar','Close')}</Button>
