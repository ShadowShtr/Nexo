...
export default function CalendarSandbox({ driverOnly = false }: { driverOnly?: boolean }) {
  ...
  return <>
    ...
    <div className="pm-card pm-calendar"><h2>{title}</h2><FullCalendar ref={calendar} plugins={[dayGridPlugin,timeGridPlugin,listPlugin,luxonPlugin]} initialDate="2026-09-11" initialView={window.innerWidth<768?'listDay':'timeGridWeek'} locales={[pt,enGB]} locale={i18n.language==='en'?'en-gb':'pt'} timeZone={zone} firstDay={1} headerToolbar={{left:'prev,next',center:'',right:'timeGridDay,timeGridWeek,dayGridMonth,listDay'}} events={events} datesSet={info=>{setTitle(info.view.title);setDate(DateTime.fromJSDate(info.view.calendar.getDate()).setZone(zone).toISODate()!);}} eventClick={info=>{if(!driverOnly&&!info.event.id.startsWith('margin-')){setDraft(entries.find(e=>e.id===info.event.id)!);setError('');setFeedback('');}}} eventInteractive editable={false} allDaySlot={false} slotDuration="01:00:00" slotMinTime="06:00:00" slotMaxTime="23:00:00" height={620} eventTimeFormat={{hour:'2-digit',minute:'2-digit',hour12:false}}/></div>
