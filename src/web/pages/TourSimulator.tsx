...
export function TourSimulator() {
  ...
  return <Section title={t('tourTitle')}>
    ...
    <div className="pm-tour-simulator">
      <div className="pm-card pm-tour pm-tour-simulator-card"><div className="pm-simulator-card-head"><div className="pm-form-heading-icon"><img className="pm-tour-simulator-icon" src="/owner-icon-tours.webp" alt="" aria-hidden="true" /></div><div><span className="pm-eyebrow">{t('tourTitle')}</span><h3>{t('included')}</h3></div></div><p className="pm-secondary">{t('extra')}: <strong>{money(3500)}</strong></p><div className="pm-stepper"><span>{t('passengers')}</span><Button variant="secondary" aria-label={t('fewer')} disabled={passengers === 1} onClick={() => setPassengers(n => n - 1)}>−</Button><output aria-live="polite">{passengers}</output><Button variant="secondary" aria-label={t('morePassengers')} disabled={passengers === 6} onClick={() => setPassengers(n => n + 1)}>+</Button></div></div>
