  ...
  return <>
    <div className="pm-settings-top-grid">
      <Section title={t('rules')}>
        <form className="pm-card pm-form pm-rules-form" onSubmit={handleSubmit(v => setResult(Math.max(v.minimum, v.travel + v.tolerance)))} onChange={() => setResult(null)} noValidate>
          <div className="pm-form-heading"><div className="pm-form-heading-icon"><img src="/owner-icon-calendar.webp" alt="" aria-hidden="true" /></div><div><span className="pm-eyebrow">{label('Agenda','Schedule')}</span><h3>{t('rules')}</h3><p>{t('simulationNote')}</p></div></div>
      ...
      <Section title={t('policies')}>
        <div className="pm-card pm-group pm-policy-card"><div className="pm-policy-heading"><div className="pm-form-heading-icon"><img src="/owner-icon-card.webp" alt="" aria-hidden="true" /></div><div><span className="pm-eyebrow">{label('Regras atuais','Current rules')}</span><p>{label('Aplicadas a novas marcações.','Applied to new bookings.')}</p></div></div><Row label={t('deposit')} value="25%"/><Row label={t('balance')} value="75%"/><Row label={t('cancellation')} value={t('cancellationValue')}/><Row label={t('leadTime')} value={t('leadTimeValue')}/><Row label={t('accept')} value={t('acceptValue')}/></div>
      </Section>
    </div>
    <Section title={label('Simulador e configuração de preço','Price simulator and configuration')}>
      <form className="pm-card pm-form pm-price-form" aria-label={label('Simulador de preço','Price simulator')} onSubmit={simulatePrice}>
        <div className="pm-form-heading"><div className="pm-form-heading-icon"><img src="/owner-icon-wallet.webp" alt="" aria-hidden="true" /></div><div><span className="pm-eyebrow">{label('Tarifas','Pricing')}</span><h3>{label('Simule antes de publicar','Simulate before publishing')}</h3><p>{label('Ajuste os valores e veja imediatamente o total, sinal e saldo.','Adjust values and preview the total, deposit and balance.')}</p></div></div>
