...
export function SettingsPage() {
  ...
  const saveTariff = (form: HTMLFormElement) => {
    ...
    const extraCents = Math.round(number('extra') * 100);
    ...
    saveDemoTariff({ ...current, ...(service === 'tour' ? { tourBaseCents: baseCents, tourExtraPassengerCents: extraCents } : { transferBaseCents: baseCents, transferCentsPerKm: perKmCents }), waitingCentsPerHour: waitingCents, nightSurchargeBps: nightBps });
    ...
    <Section title={label('Simulador e configuração de preço','Price simulator and configuration')}>
      <form className="pm-card pm-form pm-price-form" aria-label={label('Simulador de preço','Price simulator')} onSubmit={simulatePrice}>
        ...
        <div className="pm-price-field-grid pm-form-grid">
          <label className="pm-form-field-wide">{label('Serviço','Service')}<select name="service" defaultValue="transfer"><option value="transfer">Transfer</option><option value="tour">Tour</option></select></label>
          <label>{label('Passageiros','Passengers')}<input name="passengers" type="number" min="1" max="6" defaultValue="2" required /></label>
          <label>{label('Preço base (€)','Base price (€)')}<input name="base" type="number" min="0" step="0.01" defaultValue={(configured.transferBaseCents / 100).toFixed(2)} required /></label>
          <label>{label('Distância (km)','Distance (km)')}<input name="distance" type="number" min="0" step="0.1" defaultValue="35" required /></label>
          <label>{label('Preço por km (€)','Price per km (€)')}<input name="perKm" type="number" min="0" step="0.01" defaultValue={(configured.transferCentsPerKm / 100).toFixed(2)} required /></label>
          ...
          <label>{label('Adicional por pessoa / tour (€)','Additional passenger / tour (€)')}<input name="extra" type="number" min="0" step="0.01" defaultValue={(configured.tourExtraPassengerCents / 100).toFixed(2)} required /></label>
        </div>
