...
type Request = {id:string; allocation:Allocation; driver:number; car:number; route:string; people:number; name:string; total:number; deposit:number; balance:number; cancelled:boolean; rescheduled:boolean};
...
export default function CustomerSandbox({ page }: {page:string}) {
  ...
  const reschedule=(r:Request)=>{const eligibility=changeEligibility(clock,r.allocation.startsAt,r.allocation.startsAt);if(!eligibility.rescheduleEligible){setLookupError(say('Fora da janela de 24 horas.','Outside the 24-hour window.'));return;}const allocation={...r.allocation,startsAt:DateTime.fromISO(r.allocation.startsAt).plus({hours:1}).toUTC().toISO()!,endsAt:DateTime.fromISO(r.allocation.endsAt).plus({hours:1}).toUTC().toISO()!};const result=checkSchedule(allocation,[...seed,...requests.filter(item=>item.id!==r.id).map(item=>item.allocation)],clock,{minimumGapMinutes:60,delayAllowanceMinutes:15},()=>30);if(!result.available){setLookupError(say('Novo horario indisponivel.','New time unavailable.'));return;}const next={...r,allocation,rescheduled:true};requests=requests.map(item=>item.id===r.id?next:item);setHistory([...requests]);if(current?.id===r.id)setCurrent(next);if(lookupResult?.id===r.id)setLookupResult(next);setLookupError('');};
  const record=(r:Request)=><article className="pm-card pm-demo-record" key={r.id}><h2>{r.id}</h2><p role="status">{r.cancelled?say('Pedido de teste cancelado','Test request cancelled'):say('A aguardar aceitação do motorista','Awaiting driver acceptance')}</p><p>{r.route}</p><p>{DateTime.fromISO(r.allocation.startsAt).setZone('Europe/Lisbon').toFormat('dd/MM/yyyy HH:mm')} · {drivers[r.driver]} · {cars[r.car].name}</p><Row label={say('Passageiros','Passengers')} value={r.people}/><Row label={say('Total','Total')} value={money(r.total)}/><Row label={say('Sinal após aceitação · 25%','Deposit after acceptance · 25%')} value={money(r.deposit)}/><Row label={say('Saldo no início','Balance at pickup')} value={money(r.balance)}/><p>{say('Ainda não foi pago qualquer valor. O sinal só será pedido depois do aceite do motorista.','Nothing has been paid. The deposit is requested only after driver acceptance.')}</p>{!r.cancelled&&<Button variant="secondary" onClick={()=>{const next={...r,cancelled:true,allocation:{...r.allocation,status:'cancelled' as const}};requests=requests.map(item=>item.id===r.id?next:item);setHistory([...requests]);if(current?.id===r.id)setCurrent(next);if(lookupResult?.id===r.id)setLookupResult(next);}}>{say('Cancelar pedido de teste','Cancel test request')}</Button>}{!r.cancelled&&<Button variant="secondary" disabled={!changeEligibility(clock,r.allocation.startsAt,r.allocation.startsAt).rescheduleEligible} onClick={()=>reschedule(r)}>{say('Pedir reagendamento (+1h)','Request reschedule (+1h)')}</Button>}</article>;
  return <>
    ...
    <p className="pm-step-progress"><span>{say('Passo','Step')} {step}/4</span><span aria-hidden="true">·</span><strong>{['',say('Motorista e carro','Driver and car'),say('A sua viagem','Your trip'),say('Os seus dados','Your details'),say('Rever pedido','Review request')][step]}</strong></p>
    <form className="pm-card pm-demo-form" onSubmit={event=>{event.preventDefault();if(step===2&&!validate())return;if(step===4){const allocation=validate();if(!allocation)return;const r:Request={id:allocation.id,allocation,driver,car,route:selectedRoute.name,people,name,total:price.totalCents,deposit:price.depositCents,balance:price.balanceCents,cancelled:false,rescheduled:false};requests=[...requests,r];setHistory(requests);setCurrent(r);return;}setError('');setStep(step+1);}}>
