...
export default function CustomerSandbox({ page }: { page: string }) {
  ...
  const previewRoute = labelledRoute(baseRoute, origin || baseRoute.points[0].label, destination || baseRoute.points[baseRoute.points.length - 1].label);
  const price = quote({
    passengers: people,
  ...
  const requestRoute = (request: Request) => labelledRoute(request.service === 'tour' ? tourRoute : transferRoutes[request.routeIndex], request.origin, request.destination);
  ...
  return <>
    ...
    {page === 'lookup' ? <><form className="pm-card pm-demo-form" aria-label={say('Consultar reserva', 'Find booking')} onSubmit={event => { event.preventDefault(); const normalized = normalizeBookingReference(lookupCode); const found = normalized ? history.find(item => item.id === normalized) : null; if (!found) { setLookupError(say('Código não encontrado nesta sessão de teste.', 'Code not found in this test session.')); setLookupResult(null); return; } setLookupError(''); setLookupResult(found); }}><label>{say('Código de confirmação', 'Confirmation code')}<input value={lookupCode} onChange={event => setLookupCode(event.target.value)} placeholder="CLIENT-XXXXXXXX" autoComplete="off"/></label>{lookupError && <p role="alert">{lookupError}</p>}<div className="pm-actions"><Button type="submit">{say('Consultar', 'Look up')}</Button>{lookupResult && <Button type="button" variant="secondary" onClick={() => { setLookupResult(null); setLookupCode(''); setLookupError(''); }}>{say('Limpar consulta', 'Clear lookup')}</Button>}</div></form>{lookupResult && <><RouteMap route={requestRoute(lookupResult)} language={i18n.language === 'en' ? 'en' : 'pt'}/>{record(lookupResult)}</>}<h2>{say('Os meus pedidos de teste', 'My test requests')}</h2>{history.length ? history.map(record) : <p>{say('Ainda não criou pedidos nesta sessão.', 'No requests created in this session yet.')}</p>}<Button asChild><a href="#/customer/booking">{say('Marcar viagem', 'Book a trip')}</a></Button></> : current ? <><RouteMap route={requestRoute(current)} language={i18n.language === 'en' ? 'en' : 'pt'}/>{record(current)}<Button asChild><a href="#/customer/lookup">{say('Consultar pedidos', 'View requests')}</a></Button></> : <>
