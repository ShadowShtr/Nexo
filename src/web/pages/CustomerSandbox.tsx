...
const routes = [
  {name:'Aeroporto de Lisboa → Cascais',meters:35000,minutes:90},
  {name:'Lisboa → Sintra',meters:30000,minutes:90},
  {name:'Lisboa → Setúbal',meters:50000,minutes:90},
];
...
export default function CustomerSandbox({ page }: {page:string}) {
  ...
  const price=quote({passengers:people,passengerCapacity:cars[car].capacity,service:service==='tour'?{kind:'tour',baseCents:20000,extraPassengerCents:3500}:{kind:'transfer',baseCents:1000,distanceMeters:routes[route].meters,centsPerKm:200},extras:[{code:'waiting',cents:waitingCents(wait,0,15,2400)}]});
  const validate=():Allocation|null=>{
    ...
    const candidate:Allocation={id:`CLIENT-${crypto.randomUUID().slice(0,8)}`,driverId:String(driver),vehicleId:String(car),startsAt:date.toUTC().toISO()!,endsAt:date.plus({minutes:(service==='tour'?240:routes[route].minutes)+wait}).toUTC().toISO()!,status:'requested',holdExpiresAt:DateTime.fromISO(clock).plus({minutes:30}).toISO()!};
  ...
  return <>
    ...
    {page==='lookup'?<><h2>{say('Os meus pedidos de teste','My test requests')}</h2>{history.length?history.map(record):<p>{say('Ainda não criou pedidos nesta sessão.','No requests created in this session yet.')}</p>}<Button asChild><a href="#/customer/booking">{say('Marcar viagem','Book a trip')}</a></Button></>:current?<>{record(current)}<Button asChild><a href="#/customer/lookup">{say('Consultar pedidos','View requests')}</a></Button></>:<>
    ...
    <form className="pm-card pm-demo-form" onSubmit={event=>{event.preventDefault();if(step===2&&!validate())return;if(step===4){const allocation=validate();if(!allocation)return;const r:Request={id:allocation.id,allocation,driver,car,route:service==='tour'?'Sintra · Cabo da Roca':routes[route].name,people,name,total:price.totalCents,deposit:price.depositCents,balance:price.balanceCents,cancelled:false};requests=[...requests,r];setHistory(requests);setCurrent(r);return;}setError('');setStep(step+1);}}>
