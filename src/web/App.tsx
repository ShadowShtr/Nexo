...
function Language() {
  ...
  return <label className="pm-language"><Globe2 size={17}/><span className={`pm-language-flag pm-language-flag-${i18n.language === 'en' ? 'en' : 'pt'}`} aria-hidden="true"/><span className="pm-sr-only">{t('language')}</span><select aria-label={t('language')} value={i18n.language} onChange={e => { const language = e.target.value; void i18n.changeLanguage(language); const url = new URL(window.location.href); url.searchParams.set('lang', language); history.replaceState(null, '', url); }}><option value="pt-PT">Português</option><option value="en">English</option></select></label>;
}
