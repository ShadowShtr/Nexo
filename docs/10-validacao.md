...
A aba Reservas do proprietário passou a oferecer uma marcação manual com o mesmo modelo de entrada de rota do cliente: localização atual, origem e destino editáveis, pesquisa de moradas novas, paragens, rota completa no mapa, tours ativos do catálogo, data/hora, passageiros, motorista e carro. A data/hora e o recurso escolhido são validados contra a agenda do proprietário; a marcação guardada permanece visível na agenda e respeita a margem de uma hora. Build e 9 testes browser aprovados; a sincronização real entre computadores continua pendente.
